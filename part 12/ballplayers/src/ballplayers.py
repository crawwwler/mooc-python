...
def most_goals(ballplayers: list):
    return max(ballplayers, key= lambda bp: bp.goals).name

def most_points(ballplayers: list):
    x = [(bp.name, bp.number, bp.goals + bp.passes) for bp in ballplayers]
    return max(x, key= lambda bp: bp[2])[0],max(x, key= lambda bp: bp[2])[1]
